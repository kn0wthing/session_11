...
class BPETokenizer:
    def __init__(self, vocab_size=5000):
        ...
        self.itos = {}
        self.data = []
        # Add space token first, followed by other special tokens
    ...
    def initialize_vocab(self, text):
        """Initialize vocabulary with consistent space handling"""
        # Preprocess text first
        text = preprocess_hindi_text(text)
        
        # Get unique characters
        chars = sorted(list(set(text.replace(" ", ""))))  # Remove spaces from char set
        
        # Create initial vocabulary with special tokens first
        ...
        # Initial encoding - add spaces before tokens
        self.data = []
        current_pos = 0
        text_len = len(text)
        ...
        while current_pos < text_len:
            ...
            if char in self.stoi:
                # Add space token before non-space characters if needed
                if current_pos == 0 or text[current_pos - 1] == " ":
                    self.data.append(self.stoi[" "])
                self.data.append(self.stoi[char])
            current_pos += 1
    ...
    def encode(self, text: str):
        """Convert text to token indices with consistent space handling"""
        ...
        while current_pos < text_len:
            ...
            for token, idx in sorted(self.stoi.items(), key=lambda x: len(x[0]), reverse=True):
                if text[current_pos:].startswith(token):
                    if len(token) > longest_length:
                        # Prefer tokens with leading space when at word boundary
                        if current_pos == 0 or text[current_pos - 1] == " ":
                            if token.startswith(" "):
    ...
    def decode(self, token_ids: list) -> str:
        """Convert token indices back to text with improved space handling"""
        decoded_tokens = []
        ...
        for idx in token_ids:
            try:
                ...
                if idx in self.itos:
                    token = self.itos[idx]
                    if token not in self.special_tokens[1:]:  # Allow space token
...
def preprocess_hindi_text(text):
    """
    Preprocesses Hindi text with consistent space handling and normalization.
    """
    # Remove <unk> tokens first
    text = text.replace("<unk>", "")
    
    # Normalize spaces - convert all whitespace to single space
    text = re.sub(r'\s+', ' ', text)
    
    # Add spaces around punctuation (like GPT tokenizers)
    ...
    # Normalize full stops
    text = re.sub(r"।", ".", text)
    
    # Normalize spaces again after all replacements
    text = re.sub(r'\s+', ' ', text)
