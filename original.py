...
class BPETokenizer:
    def __init__(self, vocab_size=4500):
    ...
    def encode(self, text: str):
        """Convert text to token indices using greedy longest match"""
        ...
        # Process the entire text without pre-splitting
        current_pos = 0
        text_len = len(text)
    ...
    def decode(self, token_ids: list) -> str:
        """Convert token indices back to text with improved token stitching"""
        decoded_tokens = []
            ...
            try:
                ...
                if idx in self.itos:
                    ...
                    if token not in self.special_tokens:
                        if token.strip():
                            # Check if current token should be joined with previous token
                            if (decoded_tokens and (
                                # Check for combining characters and diacritical marks
                                any(c in '\u0900-\u0963\u0981-\u09FC' for c in token[0]) or
                                # Check for specific Hindi combining characters
                                token[0] in ('्', 'ा', 'ि', 'ी', 'ु', 'ू', 'े', 'ै', 'ो', 'ौ', 'ं', 'ः') or
                                # Check for hyphen
                                token.startswith('-') or
                                # Check if previous token ends with Virama
                                (decoded_tokens[-1].endswith('्'))
                            )):
                                decoded_tokens[-1] = decoded_tokens[-1] + token
                            else:
                                # Check if this token ends with Virama
                                if token.endswith('्'):
                                    decoded_tokens.append(token)
                                else:
                                    # Add space only if next token shouldn't be combined
                                    decoded_tokens.append(token)
        ...
        result = re.sub(r'([^\u0900-\u097F\u0981-\u09FC्-])([^\u0900-\u097F\u0981-\u09FC])', r'\1 \2', result)
        return result.strip()
